CREATE DATABASE IF NOT EXISTS Library_Management_System_Amit;
USE Library_Management_System_Amit;
...
`Name` VARCHAR(178),
Birth_Date DATE,
Nationality VARCHAR(178)
);

CREATE TABLE Catalog(
Catalog_ID INT PRIMARY KEY,
`Name` VARCHAR(178),
Location VARCHAR(178)
);

CREATE TABLE Genre(
Genre_ID INT PRIMARY KEY,
`Name` VARCHAR(178),
Description VARCHAR(178)
);

CREATE TABLE Staff(
Staff_ID INT PRIMARY KEY,
`Name` VARCHAR(178),
Contact_Info VARCHAR(178),
Job_Title VARCHAR(178),
Hire_Date DATE
);
...
(6, 'Pride and Prejudice', '1813-10-18', 3, 1),
...
(17, 'The Catch-22', '1961-11-10', 7, 1),
...
WHERE M.Material_ID = B.Material_ID AND B.Due_Date < '2023-04-01' AND B.Return_Date IS NULL ;
...
SELECT Material.Title, COUNT(*) AS Count_of_Borrow
...
SELECT A.Name, COUNT(*) AS Count_of_Materials_Written
FROM Author AS A, Authorship
WHERE A.Author_ID = Authorship.Author_ID AND A.Name = 'Lucas Piki';
...
SELECT g.Genre_ID, g.Name AS Genre_Name, COUNT(*) AS Total_Borrowed
FROM Material m
...
SELECT A.Author_ID, A.Name AS Author_Name, M.Title, COUNT(*) AS Materials_Borrowed_From_09_2020_10_2020
FROM Borrow AS B
JOIN Material AS M ON B.Material_ID = M.Material_ID
JOIN Authorship ON M.Material_ID = Authorship.Material_ID
JOIN Author AS A ON Authorship.Author_ID = A.Author_ID
WHERE B.Borrow_Date >= '2020-09-01' AND B.Borrow_Date <= '2020-10-31'
GROUP BY A.Author_ID, A.Name, M.Title;
...
SELECT B.Borrow_ID, M.Title, B.Return_Date
FROM Borrow AS B
JOIN Material AS M ON B.Material_ID = M.Material_ID
...
#10.	How do you add the following material to the database? Title: New book
#Date: 2020-08-01
#Catalog: E-Books
...
VALUES (35, (SELECT Material_ID FROM Material WHERE Title = 'New book'),
